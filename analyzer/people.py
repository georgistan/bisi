...
for i in indices:
    ...
    person_image = image[round(y):round(y + h), round(x):round(x + w)]

    person_id = str(uuid.uuid4()) + datetime.datetime.now().strftime("%Y%m%d%H%M%S")
    output_path = os.path.join(args.output_dir, f"person{person_id}.jpg")
